...
%% Numerical Experiment: MJLS Switched Stability (Unstable Subsystems)
% This script demonstrates another classic counterexample in hybrid systems:
% An autonomous Markov Jump Linear System (MJLS) where EACH individual mode
% is unstable (eigenvalues outside the unit circle), but the switched system
% under Markov switching is stochastically STABLE (Mean Square Stable - MSS).
%
...
%% 1. System Parameters
% Mode 1 matrix: Unstable (eigenvalues are 1.5 and 0.5)
A1 = [1.5, 0.0; 
      0.0, 0.5];

% Mode 2 matrix: Unstable (eigenvalues are 0.5 and 1.5)
A2 = [0.5, 0.0; 
      0.0, 1.5];
...
% Deterministic switching check: x(k+2) = A2 * A1 * x(k)
A_switched = A2 * A1;
disp('Eigenvalues of the product A2 * A1 (deterministic alternation):');
disp(eig(A_switched));
if all(abs(eig(A_switched)) < 1)
    disp('A2*A1 is stable! Deterministic periodic switching will converge.');
end
...
% Simulate pure Mode 1 response: x(k+1) = A1 * x(k) (Explodes!)
% Simulate pure Mode 2 response: x(k+1) = A2 * x(k) (Explodes!)
num_steps=10;
...
h_sw = plot(0:num_steps, sqrt(sum(X_runs(:, :, 1).^2, 1)), 'Color', [0.75, 0.75, 0.75], 'LineWidth', 1.0);
for r = 2:num_plot_runs
    plot(0:num_steps, sqrt(sum(X_runs(:, :, r).^2, 1)), 'Color', [0.75, 0.75, 0.75], 'LineWidth', 1.0);
end
% Plot individual mode responses (no switching)
h_m1 = plot(0:num_steps, sqrt(sum(x_mode1.^2, 1)), 'b--', 'LineWidth', 2.5);
h_m2 = plot(0:num_steps, sqrt(sum(x_mode2.^2, 1)), 'k:', 'LineWidth', 2.5);

grid on;
title('Individual Realizations vs. Pure Modes');
xlabel('Time step (k)');
ylabel('State Norm ||x_k||');
legend([h_sw, h_m1, h_m2], {'Switched Runs', 'Pure Mode 1 (Unstable)', 'Pure Mode 2 (Unstable)'}, 'Location', 'NorthEast');
set(gca, 'FontSize', 11);
...
sgtitle('Stochastic Stability of MJLS with Unstable Subsystems', 'FontSize', 14, 'FontWeight', 'bold');
...
saveas(gcf, fullfile(images_dir, 'mjls_stability.png'));

%% 7. Theoretical Stability Verification (switching operator)
...
if spectral_radius < 1
    disp('Theoretical Result: The system is Mean Square Stable (spectral radius < 1).');
else
    disp('Theoretical Result: The system is stochastically UNSTABLE.');
end
