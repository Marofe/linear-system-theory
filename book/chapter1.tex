...
\end{itemize}
\end{mydefinition}
We can represent the continuous-time LTI state-space equations using the block diagram shown in Figure \ref{fig:modelo_ss}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{modelo_ss.png}
    \caption{Continuous-time state-space block diagram representation.}
    \label{fig:modelo_ss}
\end{figure}
...
\end{mydefinition}
Symmetrically, the discrete-time LTI state-space equations are represented in block diagram form in Figure \ref{fig:modelo_ss_discreto}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{modelo_ss_discreto.png}
    \caption{Discrete-time state-space block diagram representation.}
    \label{fig:modelo_ss_discreto}
\end{figure}

\section{Linearization of Multi-Input Multi-Output (MIMO) Systems}
Most physical systems are fundamentally nonlinear. A classic example is the inverted pendulum on a cart, as shown in Figure \ref{fig:pendulo_invertido}. However, local analysis and controller design can be performed by linearizing the nonlinear dynamics about a desired equilibrium point.

\begin{figure}[H]
    \centering
    \includegraphics[width=5cm]{pendulo_invertido.png}
    \caption{Physical model of an inverted pendulum system on a cart.}
    \label{fig:pendulo_invertido}
\end{figure}
...
\end{equation}
This local approximation is conceptually illustrated in Figure \ref{fig:linearizacao}, where a nonlinear curve is locally approximated by its tangent line.

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{linearizacao.png}
    \caption{Linearization of a nonlinear function about an operating point.}
...
Calculating the matrix exponential numerically is a highly non-trivial task. In their landmark paper \textit{"Nineteen Dubious Ways to Compute the Exponential of a Matrix"} (SIAM Review, 1978 and 2003 updates), Cleve Moler and Charles Van Loan explored the mathematical and numerical properties of 19 different methods (including series approximation, ODE solvers, polynomial methods, and matrix decomposition), highlighting issues of numerical stability, stiffness, and floating-point errors. Moler and Van Loan explored these numerical challenges in their original 1978 paper (Figure \ref{fig:dubious_1978}) and the 2003 retrospective update (Figure \ref{fig:dubious_2003}), collectively shown in Figure \ref{fig:dubious_papers}.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{19_dubious_way_exp.png}
        \caption{Original 1978 paper on computing the matrix exponential.}
        \label{fig:dubious_1978}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{19_dubious_way_exp_25_years_later.png}
        \caption{25 years later retrospective update (2003).}
        \label{fig:dubious_2003}
    \end{minipage}
    \caption{Landmark papers by Moler and Van Loan addressing the challenges in computing matrix exponentials.}
    \label{fig:dubious_papers}
\end{figure}
