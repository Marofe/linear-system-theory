\chapter{Markov Jump Linear Systems}
...
- **Fault-tolerant systems**: Sudden component failures change the control paths.
- **Flight control**: Aircraft shifting between different subsonic and supersonic regimes.
- **Networked Control Systems (NCS)**: Random packet dropouts and communication delays lead to switching network topologies.

A powerful mathematical framework for modeling such phenomena is the **Markov Jump Linear System (MJLS)**. An MJLS is a switched linear system where the switching between different operating modes is governed by a stochastic process called a Markov chain.
...
In the stochastic setting, we must define stability in a statistical sense. The most common and useful notion for MJLS is **Mean Square Stability (MSS)**.
...
\end{equation}
Using dynamic programming, the mode-dependent feedback gains $K_i$ and the optimal cost-to-go matrices $P_i$ are found by solving the **Coupled Discrete Algebraic Riccati Equations (CDARE)**.
...
A critical application of MJLS is the modeling and control of **Networked Control Systems (NCS)**. In an NCS, the control loop is closed through a digital communication network. Transmission delays and packet dropouts occur randomly due to network traffic congestion and wireless interference.
...
- **Mode 1 (Transmission Success)**: The packet is transmitted successfully. The controller updates the input: $u_k = -K x_k$. The dynamics are governed by the closed-loop matrix $A_1 = A - BK$.
- **Mode 2 (Transmission Failure)**: The packet is dropped. The ZOH holds the previous control signal constant: $u_k = u_{k-1}$. The system is unforced or operates under a delayed feedback loop, yielding $A_2 = A$.
