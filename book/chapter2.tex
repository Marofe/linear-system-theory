\chapter{Similarity and Jordan Forms}
...
A defective matrix is transformed into a block-diagonal matrix where each diagonal element is a \textbf{Jordan Block}.
...
\end{equation}
\end{mydefinition}
To construct the similarity transformation matrix $T$ for a Jordan Block, we must calculate \textbf{generalized eigenvectors}.
...
(A - \lambda I) v_1 = 0 ,
    \end{equation}
    \item The subsequent generalized eigenvectors $v_2, \dots, v_k$ are calculated recursively, forming a \textbf{Jordan Chain}:
    \begin{align}
        (A - \lambda I) v_2 &= v_1 \\
        (A - \lambda I) v_3 &= v_2 \\
        \vdots \nonumber \\
