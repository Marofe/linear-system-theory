...
\end{equation}
The discrete-time state-space dynamics can be visualized schematically in Figure \ref{fig:ss_discrete_ch3}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{modelo_ss_discreto.png}
    \caption{Discrete-time state-space representation layout.}
    \label{fig:ss_discrete_ch3}
\end{figure}
...
\end{equation}
we see that a target state $x_k$ is reachable from $x_0$ if and only if the vector $x_k - A^k x_0$ lies in the column space (range) of $\mathcal{C}_k$, a geometric relationship illustrated in Figure \ref{fig:controlabilidade2}.

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{controlabilidade2.png}
    \caption{State reachability in discrete-time, showing that reachable states must lie in the range of the controllability matrix.}
    \label{fig:controlabilidade2}
\end{figure}
...
\end{mydefinition}
This transition between arbitrary states in state-space is conceptually diagrammed in Figure \ref{fig:controlabilidade}.

\begin{figure}[H]
    \centering
    \includegraphics[width=6cm]{controlabilidade.png}
    \caption{Conceptual block diagram of controllability showing transitions between arbitrary states in state-space.}
    \label{fig:controlabilidade}
\end{figure}
...
\end{mytheorem}
Continuous-time controllability is conceptually shown as state trajectories moving through space in Figure \ref{fig:controlabilidade_continuo}.

\begin{figure}[H]
    \centering
    \includegraphics[width=7cm]{controlabilidade_continuo.png}
    \caption{Continuous-time state-space controllability concept and state transitions.}
    \label{fig:controlabilidade_continuo}
\end{figure}
...
A more numerically robust method to evaluate and characterize controllability is using the \textbf{Controllability Gramian}.
...
\end{equation}
which is the unique positive definite solution to the continuous-time \textbf{Lyapunov Equation}:
\begin{equation}
A W_c + W_c A^\trp + B B^\trp = 0 .
\end{equation}
The infinite-horizon Controllability Gramian characterizes the ellipsoids of reachable directions, as plotted in Figure \ref{fig:reachability_directions}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{reachability_directions.png}
    \caption{Visualization of reachability ellipsoids and subspaces in state-space.}
    \label{fig:reachability_directions}
\end{figure}
...
\end{mydefinition}
This estimation process is conceptually illustrated in Figure \ref{fig:observabilidade}, showing state reconstruction from input-output records.

\begin{figure}[H]
    \centering
    \includegraphics[width=6.5cm]{observabilidade.png}
    \caption{Conceptual block diagram of observability showing state reconstruction from input-output records.}
...
By combining controllability and observability decompositions, any LTI system can be transformed into the \textbf{Kalman Canonical Decomposition}, separating the state vector into four distinct subspaces:
\begin{enumerate}
...
\end{equation}
A state-space realization is said to be \textbf{minimal} if and only if the system is both fully controllable and fully observable.

\section{Physical Example: Uncontrollable Op-Amp Circuit}
To illustrate these abstract structural properties in a concrete physical system, consider the active Op-Amp circuit shown in Figure \ref{fig:ampOp_circuit}.

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{ampOp_sistema_non_controllable.png}
    \caption{Active Op-Amp circuit schematic featuring uncontrollable dynamics.}
    \label{fig:ampOp_circuit}
\end{figure}

Assuming the voltages across the capacitors $C_1$ and $C_2$ are the state variables $x = [v_{C1}, v_{C2}]^\trp$, we can write the state-space equations using nodal analysis. By analyzing the network under identical RC branch parameters, we find that the input voltage $u(t)$ affects both state variables in a symmetric manner that prevents independent control of each state. 

Specifically, if the bridge branches are perfectly matched, one of the modes becomes completely decoupled from the input terminal. The controllability matrix loses rank, yielding:
\begin{equation}
\text{rank}(\mathcal{C}) < 2 ,
\end{equation}
which proves that the system is uncontrollable. This demonstrates how physical symmetries in circuit layout or component values can directly lead to structural uncontrollability.
