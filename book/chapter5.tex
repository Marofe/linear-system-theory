\chapter{State Feedback and LQR}
...
\end{equation}
where $K \in \R^{m \times n}$ is the state feedback gain matrix and $r(t) \in \R^m$ is the reference input. We can represent the state feedback control loop using the block diagram in Figure \ref{fig:state_feedback}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{state_feedback.png}
    \caption{State feedback control loop block diagram.}
...
- $R = R^\trp > 0$ is the control weighting matrix, penalizing control effort. The LQR optimization trade-off between tracking speed and control effort is conceptually illustrated in Figure \ref{fig:controle_otimo}.

\begin{figure}[H]
    \centering
    \includegraphics[width=6.5cm]{controle_otimo.png}
    \caption{The LQR optimization trade-off between tracking speed and control effort.}
    \label{fig:controle_otimo}
\end{figure}
...
\end{proof}
The quadratic cost profile $V(x) = x^\trp P x$ forms a convex potential well in the state space, as shown in Figure \ref{fig:lqr_cost_profile}.

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{LQR_cost.png}
    \caption{The LQR infinite-horizon cost function profile in the state space.}
    \label{fig:lqr_cost_profile}
\end{figure}
...
The discrete LQR optimization trajectory planning and constraints are conceptually illustrated in Figures \ref{fig:opt_trajectory}, \ref{fig:opt_states}, and \ref{fig:opt_controls} (collectively shown in Figure \ref{fig:discrete_opt_concept}), while the sequence of optimal inputs driving the state trajectories is plotted in Figure \ref{fig:controle_otimo4}.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo.png}
        \caption{Optimal trajectory optimization formulation.}
        \label{fig:opt_trajectory}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo2.png}
        \caption{State evolution and terminal state constraints.}
        \label{fig:opt_states}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo3.png}
        \caption{Control action search space constraints.}
        \label{fig:opt_controls}
    \end{minipage}
    \caption{Discrete-time optimal control trajectory planning concept and constraints.}
    \label{fig:discrete_opt_concept}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{controle_otimo4.png}
    \caption{Sequence of optimal control inputs driving states toward the terminal manifold.}
    \label{fig:controle_otimo4}
\end{figure}
...
\end{equation}
This recursive sub-path optimization process is shown in Figures \ref{fig:bellman_path1}, \ref{fig:bellman_path2}, and \ref{fig:bellman_path3} (collectively shown in Figure \ref{fig:bellman_principle}).

\begin{figure}[H]
    \centering
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo_bellman.png}
        \caption{Complete trajectory optimization path.}
        \label{fig:bellman_path1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo_bellman2.png}
        \caption{Optimal sub-path formulation starting from step $k$.}
        \label{fig:bellman_path2}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{controle_otimo_bellman3.png}
        \caption{Bellman recursive sub-optimization partition.}
        \label{fig:bellman_path3}
    \end{minipage}
    \caption{Graphical explanation of Bellman's Principle of Optimality for discrete-time systems.}
    \label{fig:bellman_principle}
\end{figure}
...
\end{align}
The stage cost and cost-to-go structures in the Bellman equation are illustrated in Figures \ref{fig:custo_estagio} and \ref{fig:custo_to_go} (collectively shown in Figure \ref{fig:bellman_costs}).

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{custo_estagio.png}
        \caption{Stage cost representation (instantaneous penalty).}
        \label{fig:custo_estagio}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{custo_to_go.png}
        \caption{Optimal cost-to-go function representation (accumulated future cost).}
        \label{fig:custo_to_go}
    \end{minipage}
    \caption{Stage cost vs. cost-to-go visual structures in the Bellman equation.}
    \label{fig:bellman_costs}
\end{figure}
...
\end{equation}
These different classes of Lyapunov functions are shown geometrically in Figures \ref{fig:lyap_dep}, \ref{fig:lyap_const}, and \ref{fig:lyap_affine} (collectively shown in Figure \ref{fig:lyap_spaces}).

\begin{figure}[H]
    \centering
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{espaco_Palpha_funcoes.png}
        \caption{Parameter-dependent Lyapunov function space.}
        \label{fig:lyap_dep}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{espaco_Pcte_funcoes.png}
        \caption{Common quadratic (constant $P$) Lyapunov space.}
        \label{fig:lyap_const}
    \end{minipage}
    \hfill
    \begin{minipage}{0.31\textwidth}
        \centering
        \includegraphics[width=\textwidth]{espaco_Pafim_funcoes.png}
        \caption{Affine Lyapunov function parameter space.}
        \label{fig:lyap_affine}
    \end{minipage}
    \caption{Geometric representations of parameter spaces for different classes of Lyapunov stability functions in uncertain systems.}
    \label{fig:lyap_spaces}
\end{figure}
...
\section{Case Study: Inverted Pendulum State Feedback Control}
To demonstrate state feedback design in practice, consider the classic inverted pendulum on a cart system. The system's mechanical schematic and control loops are shown in Figures \ref{fig:pendulo_planta} and \ref{fig:pendulo_sf}.
Specifically, the physical forces and kinematic coordinates are shown in Figures \ref{fig:pendulo_forces} and \ref{fig:pendulo_kinematics}, while the linearized parameters and state feedback structure are illustrated in Figures \ref{fig:pendulo_linearized} and \ref{fig:pendulo_sf_structure}.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pendulo_invertido_planta.png}
        \caption{Physical forces acting on the cart and pendulum.}
        \label{fig:pendulo_forces}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pendulo_invertido_planta2.png}
        \caption{Kinematic relationships and coordinates.}
        \label{fig:pendulo_kinematics}
    \end{minipage}
    \caption{Physical schematics of the inverted pendulum system.}
    \label{fig:pendulo_planta}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pendulo_invertido_planta3.png}
        \caption{Linearized state space equations and parameter values.}
        \label{fig:pendulo_linearized}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pendulo_invertido_planta_statefeedback.png}
        \caption{State feedback loop structure with reference signal.}
        \label{fig:pendulo_sf_structure}
    \end{minipage}
    \caption{Linearized models and state-feedback configurations.}
    \label{fig:pendulo_sf}
\end{figure}

Using the state-space matrices of the linearized pendulum, we can perform pole placement or design an optimal LQR controller. The full state-feedback gains ensure that the cart stabilizes at its reference position while balancing the pendulum vertically in its unstable upright equilibrium.

\section{Homework: Discrete-Time Polytopic LQR Design}
To consolidate these optimal and robust control concepts, consider the inverted pendulum plant represented as a discrete-time uncertain polytopic system $(A, B)(\alpha)$.
The model of the inverted pendulum is shown in Figure \ref{fig:homework_pendulum}.

\begin{figure}[H]
    \centering
    \includegraphics[width=5cm]{pendulum.png}
    \caption{Dynamic model of the inverted pendulum for numerical synthesis.}
    \label{fig:homework_pendulum}
\end{figure}

\begin{exercise}
Design a discrete-time LQR controller for the nominal inverted pendulum. Next, assume a $\pm 20\%$ parametric uncertainty in the mass of the pendulum, formulating a polytopic uncertain model with 2 vertex matrices. Using MATLAB with YALMIP and SeDuMi, solve the robust LMI synthesis problem to find a single stabilizing state feedback gain $K = Y W^{-1}$. Compare the robust closed-loop transient responses against the nominal LQR design under parameter variations.
\end{exercise}
