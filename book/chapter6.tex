\chapter{State Observers and The Kalman Filter}
...
To overcome this, we design a dynamic system called a **state observer** (or estimator) that runs in real-time in the control computer. The observer processes the inputs $u(t)$ and outputs $y(t)$ to generate an estimate of the state vector, denoted by $\hat{x}(t) \in \R^n$.
...
\end{equation}
This combined controller-observer structure is called an **output feedback controller**. Since the controller relies on estimates and the estimator relies on the control input, they are coupled. We must prove that this coupled system remains stable and that the controller and observer gains can be designed independently.
...
\end{equation}
This proves that the closed-loop poles are precisely the poles of $A - B K$ together with the poles of $A - L C$. The design of the controller and the design of the observer are completely **separated**! We can design $K$ to meet transient specifications assuming full state feedback, and design $L$ to ensure the estimation error converges rapidly, without any risk of coupled instability.
\end{proof}
...
The **Kalman Filter**, introduced by Rudolf E. Kálmán in 1960, is the mathematically optimal estimator for linear systems subjected to Gaussian white noise.
...
\end{align}
where:
- $w_k \in \R^n$ is the **process noise**, representing random unmodeled disturbances.
- $v_k \in \R^p$ is the **measurement noise**, representing sensor inaccuracies.
...
- $\hat{x}_{k|k-1}$: The **a priori** state estimate at step $k$, given all measurements up to step $k-1$.
- $\hat{x}_{k|k}$: The **a posteriori** state estimate at step $k$, given all measurements up to step $k$.
- $P_{k|k-1}$: The **a priori** estimation error covariance matrix:
  \begin{equation}
      P_{k|k-1} = \mathbb{E}\left[ (x_k - \hat{x}_{k|k-1})(x_k - \hat{x}_{k|k-1})^\trp \right]
  \end{equation}
- $P_{k|k}$: The **a posteriori** estimation error covariance matrix:
  \begin{equation}
      P_{k|k} = \mathbb{E}\left[ (x_k - \hat{x}_{k|k})(x_k - \hat{x}_{k|k})^\trp \right]
  \end{equation}

The filter operates in two alternating phases: **Prediction (Time Update)** and **Correction (Measurement Update)**.
...
\end{equation}
where $K_k \in \R^{n \times p}$ is the **Kalman Gain** to be determined.
...
\end{equation}
Equation \eqref{eq:kf_joseph_form} is known as the **Joseph Form** of the covariance update, which is numerically stable and holds for any choice of gain $K_k$.
