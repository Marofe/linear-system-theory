...
\end{equation}
The impulse response energy channels are represented in Figure \ref{fig:impulse_response_diagram}, while their trajectories are shown in Figure \ref{fig:impulse_response_continuous}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{impulse_response_diagram.png}
    \caption{Visual representation of H2 norm channel impulse response evaluation.}
    \label{fig:impulse_response_diagram}
\end{figure}
...
The state trajectory responses to continuous impulse disturbances are shown in Figure \ref{fig:impulse_response_continuous}.

\begin{figure}[H]
    \centering
    \includegraphics[width=7.5cm]{impulse_response_continuous.png}
    \caption{State trajectory responses to impulse disturbances in continuous time.}
    \label{fig:impulse_response_continuous}
\end{figure}
...
\section{Discrete-Time H2 Norm Formulation}
For discrete-time LTI systems:
\begin{align}
    x_{k+1} &= A x_k + B_w w_k \\
z_k &= C_z x_k ,
\end{align}
the $H_2$ norm is also defined through the impulse response energy, but in discrete steps. The discrete-time impulse response trajectories are shown in Figure \ref{fig:impulse_response_discrete}.

\begin{figure}[H]
    \centering
    \includegraphics[width=8.5cm]{impulse_response_discrete.png}
    \caption{Discrete-time impulse response trajectories of system channels.}
    \label{fig:impulse_response_discrete}
\end{figure}

The squared discrete-time $H_2$ norm is computed as:
\begin{equation}
\| G \|_2^2 = \sum_{k=0}^\infty \tr\left( g_k^\trp g_k \right) = \tr(B_w^\trp P_o B_w) = \tr(C_z P_c C_z^\trp) ,
\end{equation}
where the discrete observability and controllability Gramians satisfy the discrete-time Lyapunov (Stein) equations:
\begin{align}
    A^\trp P_o A - P_o + C_z^\trp C_z &= 0 \\
A P_c A^\trp - P_c + B_w B_w^\trp &= 0 .
\end{align}
