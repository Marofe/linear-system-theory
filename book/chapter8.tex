\chapter{Robust H-infinity Control via LMI}
...
In the time domain, the $H_\infty$ norm corresponds to the **induced $L_2$-to-$L_2$ norm** of the system. Let $L_2$ be the space of square-integrable signals:
\begin{equation}
...
Evaluating the $H_\infty$ norm using the frequency-domain definition \eqref{eq:hinf_freq} requires sweeping across all frequencies, which is computationally cumbersome. The **Bounded Real Lemma (BRL)** provides a state-space characterization of the $H_\infty$ norm, formulating the inequality $\| G \|_\infty < \gamma$ as a single Linear Matrix Inequality.
