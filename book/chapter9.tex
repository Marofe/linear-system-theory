...
Under such conditions, a standard Kalman filter can lose its optimality or even diverge. To address this, \textbf{robust filtering} designs estimators that guarantee a bound on the estimation error energy, regardless of parameter variations and the exact nature of the noise, by utilizing $H_2$ and $H_\infty$ performance bounds.
...
The robust state estimation error system is shown in Figure \ref{fig:robust_state_observer}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9.5cm]{robust_state_observer.png}
    \caption{Robust state estimation filter block diagram under uncertainty.}
    \label{fig:robust_state_observer}
\end{figure}
...
The noise-attenuation performance bounds from w to e are shown in Figure \ref{fig:robust_state_observer_disturb}.

\begin{figure}[H]
    \centering
    \includegraphics[width=9.5cm]{robust_state_observer_disturb.png}
    \caption{Robust estimation filter loop with external noise and disturbance signals.}
    \label{fig:robust_state_observer_disturb}
\end{figure}
...
In many practical scenarios, we want to balance the worst-case disturbance rejection ($H_\infty$) and average-case white noise regulation ($H_2$). This is known as \textbf{multiobjective filtering}. Because both problems share the exact same optimization variable $P$ and change of variables $F = P L$, we can solve them simultaneously with a single shared Lyapunov matrix $P$, yielding a filter gain $L = P^{-1}F$ that satisfies both criteria:
\begin{equation}
